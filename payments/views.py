...
def create_payment(request, booking_id):
    ...
    if booking.status != 'confirmed':
        messages.error(request, 'Cannot process payment for unconfirmed booking.')
...
def payment_history(request):
    payments = Payment.objects.filter(user=request.user).select_related('booking', 'booking__event')
...
def request_refund(request, payment_id):
    ...
    if payment.refunds.exists():
        ...
        return redirect('refund_history')
...
def refund_history(request):
    refunds = Refund.objects.filter(payment__user=request.user).select_related('payment')
...
def stripe_webhook(request):
    # This is a simplified example - implement proper Stripe webhook handling
    try:
